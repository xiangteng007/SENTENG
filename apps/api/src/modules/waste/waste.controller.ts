...
export class WasteController {
  ...
  async createRecord(@Body() dto: CreateWasteRecordDto, @Req() req: AuthRequest) {
    return this.wasteService.createRecord(dto, req.user?.id);
  }

  @Patch('records/:id')
  @RequirePermissions('waste:update')
  async updateRecord(@Param('id') id: string, @Body() dto: UpdateWasteRecordDto) {
  ...
  @RequirePermissions('waste:update')
  async submitMonthlyReport(@Param('id') id: string, @Body() dto: SubmitMonthlyReportDto) {
