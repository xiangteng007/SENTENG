...
export class WasteService {
  ...
  async findRecords(query: WasteRecordQueryDto): Promise<WasteRecord[]> {
    ...
    if (query.wasteType)
      qb.andWhere("wr.wasteType = :wasteType", { wasteType: query.wasteType });
    if (query.status)
      qb.andWhere("wr.status = :status", { status: query.status });
    if (query.isRecyclable !== undefined)
      qb.andWhere("wr.isRecyclable = :isRecyclable", {
        isRecyclable: query.isRecyclable,
      });
    if (query.startDate)
      qb.andWhere("wr.wasteDate >= :startDate", { startDate: query.startDate });
    if (query.endDate)
      qb.andWhere("wr.wasteDate <= :endDate", { endDate: query.endDate });
  ...
  async generateMonthlyReport(
    ...
    if (existing)
      throw new BadRequestException(
        `Report for ${dto.year}/${dto.month} already exists`,
      );
    ...
    const byType = new Map<
      WasteType,
      { quantity: number; recycled: number; cost: number }
    >();
    ...
    const summary = Array.from(byType.entries()).map(([wasteType, data]) => ({
      ...
      recycleRate:
        data.quantity > 0
          ? Math.round((data.recycled / data.quantity) * 100)
          : 0,
      disposalCost: data.cost,
    }));

    const totalQuantity = summary.reduce((sum, s) => sum + s.totalQuantity, 0);
    const totalRecycled = summary.reduce(
      (sum, s) => sum + s.recycledQuantity,
      0,
    );
    ...
    const report = this.reportRepository.create({
      ...
      overallRecycleRate:
        totalQuantity > 0
          ? Math.round((totalRecycled / totalQuantity) * 100)
          : 0,
      status: "draft",
    });
  ...
  async getStatistics(
    ...
    return {
      ...
      totals: {
        ...
        recycleRate:
          totalQty > 0 ? Math.round((recycledQty / totalQty) * 100) : 0,
        disposalCost,
        transportCost,
      },
      pendingManifests,
