...
export class GoogleSheetsService {
  ...
  async exportEstimate(
    ...
    const title = request.options?.title || `估價單_${new Date().toISOString().split('T')[0]}`;
    ...
    try {
      ...
      Object.entries(categoryTotals).forEach(([category, total]) => {
        summaryData.push([this.getCategoryLabel(category), this.formatCurrency(total)]);
      });
  ...
  private async formatSpreadsheet(
    ...
    try {
      ...
      const summarySheetId = spreadsheetsInfo.data.sheets?.[0]?.properties?.sheetId || 0;
      const detailSheetId = spreadsheetsInfo.data.sheets?.[1]?.properties?.sheetId || 0;
