...
export class IfcParserService {
  ...
  async parseFile(_fileBuffer: Buffer): Promise<IfcElement[]> {
    ...
    this.logger.warn(
      'IFC parsing in POC mode - returning mock data. ' + 'Install web-ifc for actual IFC parsing.'
    );
  ...
  extractQuantities(elements: IfcElement[]): QuantityTakeoff {
    ...
    for (const el of elements) {
      switch (el.type) {
        ...
        case 'IfcDoor':
          takeoff.doors.count++;
          takeoff.doors.byType[el.name] = (takeoff.doors.byType[el.name] || 0) + 1;
          break;
        case 'IfcWindow':
          takeoff.windows.count++;
          takeoff.windows.totalArea += el.dimensions.area || 0;
          takeoff.windows.byType[el.name] = (takeoff.windows.byType[el.name] || 0) + 1;
          break;
        case 'IfcStair':
