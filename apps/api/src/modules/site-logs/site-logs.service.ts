...
export class SiteLogsService {
  ...
  async findAll(query: SiteLogQueryDto): Promise<{ items: SiteLog[]; total: number }> {
    const { page = 1, limit = 20, projectId, startDate, endDate, approved } = query;
    const qb = this.siteLogRepo.createQueryBuilder('s').leftJoinAndSelect('s.project', 'project');
    ...
    if (approved !== undefined) qb.andWhere('s.isApproved = :approved', { approved });
  ...
  async create(dto: CreateSiteLogDto, userId?: string): Promise<SiteLog> {
    ...
    if (existing) {
      throw new BadRequestException(`Site log already exists for ${dto.logDate}`);
    }
  ...
  async approve(id: string, userId: string): Promise<SiteLog> {
    const log = await this.findOne(id);
    if (!log.submittedAt) {
      throw new BadRequestException('Site log must be submitted before approval');
    }
    log.isApproved = true;
  ...
  async getProjectSummary(projectId: string, startDate?: string, endDate?: string): Promise<any> {
    ...
    return {
      ...
      totalWorkers: logs.reduce((sum, l) => sum + (l.workersOwn || 0) + (l.workersSubcon || 0), 0),
      avgWorkersPerDay: logs.length
        ? logs.reduce((sum, l) => sum + (l.workersOwn || 0) + (l.workersSubcon || 0), 0) /
          logs.length
        : 0,
      ...
      safetyIncidents: logs.reduce((sum, l) => sum + (l.safety?.incidents || 0), 0),
    };
  }
