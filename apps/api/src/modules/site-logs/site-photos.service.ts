...
import { GoogleDriveService, DriveFile } from '../integrations/google-drive.service';
...
export class SitePhotosService {
  ...
  async uploadPhoto(
    ...
    try {
      // 確保「工程照片」資料夾存在
      const photoFolderId = await this.googleDriveService.ensureProjectPhotoFolder(
        userId,
        project.driveFolder
      );
      ...
      this.logger.log(`Photo uploaded: ${savedPhoto.id} for project ${dto.projectId}`);
