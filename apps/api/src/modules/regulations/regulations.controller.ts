...
export class RegulationsController {
  ...
  async searchArticles(@Query() dto: SearchRegulationsDto) {
    return this.regulationsService.searchArticles(
      dto.query,
      dto.pcode,
      dto.limit,
    );
  ...
  async generateArticleSummary(@Param("articleId") articleId: string) {
    ...
    const summary = await this.geminiAiService.generateRegulationSummary(
      article.content,
    );
  ...
  async suggestRelatedRegulations(
    @Param("materialCategory") materialCategory: string,
  ) {
    const suggestions =
      await this.geminiAiService.suggestRelatedRegulations(materialCategory);
