...
export class BuildingCodeService {
  ...
  // 住宅區基準
  private readonly ZONE_STANDARDS: Record<
    ZoneType,
  ...
  private checkBcr(input: BuildingCheckInput): BcrCheckResult {
    const allowed =
      input.lot.bcr || this.ZONE_STANDARDS[input.lot.zoneType]?.bcr || 60;
  ...
  private checkFar(input: BuildingCheckInput): FarCheckResult {
    const allowed =
      input.lot.far || this.ZONE_STANDARDS[input.lot.zoneType]?.far || 200;
  ...
  private checkSetback(input: BuildingCheckInput): SetbackCheckResult {
    ...
    if (input.setbacks.front < frontRequired) {
      violations.push(
        `前院深度不足: 需 ${frontRequired}m，實際 ${input.setbacks.front}m`,
      );
    }
    ...
    if (input.setbacks.rear < rearRequired) {
      violations.push(
        `後院深度不足: 需 ${rearRequired}m，實際 ${input.setbacks.rear}m`,
      );
    }
    ...
    if (input.setbacks.side < sideRequired) {
      violations.push(
        `側院寬度不足: 需 ${sideRequired}m，實際 ${input.setbacks.side}m`,
      );
    }
  ...
  private checkParking(input: BuildingCheckInput): ParkingCheckResult {
    ...
    return {
      ...
      handicappedRequired,
      compliant:
        input.parking.provided >= required &&
        input.parking.handicapped >= handicappedRequired,
    };
  }
