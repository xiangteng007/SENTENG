...
export class InvoiceExportService {
  ...
  async exportToExcel(query: ExportExcelDto, userId?: string, userRole?: string): Promise<Buffer> {
    ...
    const { data: invoices } = await this.invoicesService.findAll(exportQuery, userId, userRole);
  ...
  async exportTo401(dto: Export401Dto, userId?: string, userRole?: string): Promise<string> {
    ...
    const { data: invoices } = await this.invoicesService.findAll(queryDto, userId, userRole);
