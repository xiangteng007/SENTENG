...
export class InvoicesService {
   ...
   * 取得單一發票
   */
  async findOne(id: string, userId?: string, userRole?: string): Promise<Invoice> {
  ...
  async create(dto: CreateInvoiceDto, userId?: string): Promise<Invoice> {
    ...
    // 自動判斷進項扣抵狀態
    let vatDeductibleStatus = dto.vatDeductibleStatus || VatDeductibleStatus.UNKNOWN;
    if (dto.docType === 'INVOICE_B2B' || dto.docType === 'INVOICE_EGUI') {
      if (dto.buyerTaxId && vatDeductibleStatus === VatDeductibleStatus.UNKNOWN) {
    ...
    const invoice = this.invoicesRepository.create({
      ...
      invoiceNo: dto.invoiceNo || `${dto.invoiceTrack || ''}${dto.invoiceNumber || ''}`,
      invoicePeriod,
  ...
  async update(
    ...
    // 只有草稿/待覆核狀態可修改基本資訊
    const editableStates = [InvoiceState.DRAFT, InvoiceState.NEEDS_REVIEW, InvoiceState.REVIEWED];
    ...
    // 更新金額計算
    if (updateData.amountNet !== undefined || updateData.amountGross !== undefined) {
      const vatRate = updateData.vatRate ?? invoice.vatRate;
      if (updateData.amountGross && !updateData.amountNet) {
        updateData.amountNet = Math.round(updateData.amountGross / (1 + vatRate));
    ...
    // 同步舊欄位
    if (updateData.amountNet !== undefined) invoice.subtotal = updateData.amountNet;
    if (updateData.amountTax !== undefined) invoice.taxAmount = updateData.amountTax;
    if (updateData.amountGross !== undefined) invoice.totalAmount = updateData.amountGross;
  ...
  async changeState(
    ...
    const validTransitions: Record<string, string[]> = {
      ...
      [InvoiceState.UPLOADED]: [InvoiceState.AI_EXTRACTED, InvoiceState.REVIEWED],
      [InvoiceState.AI_EXTRACTED]: [InvoiceState.NEEDS_REVIEW, InvoiceState.REVIEWED],
      [InvoiceState.NEEDS_REVIEW]: [InvoiceState.REVIEWED],
      [InvoiceState.REVIEWED]: [InvoiceState.ASSIGNED, InvoiceState.PENDING_APPROVAL],
      [InvoiceState.ASSIGNED]: [InvoiceState.PENDING_APPROVAL],
      [InvoiceState.PENDING_APPROVAL]: [InvoiceState.APPROVED, InvoiceState.REJECTED],
      [InvoiceState.APPROVED]: [InvoiceState.PAYABLE_SCHEDULED, InvoiceState.PAID],
    ...
    if (!validTransitions[currentState]?.includes(newState)) {
      throw new BadRequestException(`不允許從 ${currentState} 轉換到 ${newState}`);
    }
   ...
   * 作廢發票
   */
  async void(id: string, reason: string, userId?: string, userRole?: string): Promise<Invoice> {
    const invoice = await this.findOne(id, userId, userRole);
  ...
  async getStats(userId?: string, userRole?: string): Promise<InvoiceStatsDto> {
    try {
      ...
      const stats: InvoiceStatsDto = {
        ...
        totalAmountNet: all.reduce((sum, i) => sum + Number(i.amountNet || 0), 0),
        totalAmountTax: all.reduce((sum, i) => sum + Number(i.amountTax || 0), 0),
        totalAmountGross: all.reduce((sum, i) => sum + Number(i.amountGross || 0), 0),
        byState: {},
        byPaymentStatus: {},
        byVatStatus: {},
        needsReviewCount: all.filter(i => i.currentState === InvoiceState.NEEDS_REVIEW).length,
        pendingApprovalCount: all.filter(i => i.currentState === InvoiceState.PENDING_APPROVAL)
          .length,
        unpaidCount: all.filter(
          i => i.paymentStatus === PaymentStatus.UNPAID && i.currentState === InvoiceState.APPROVED
        ).length,
      };
  ...
  async getMonthlyStats(
    ...
    return {
      ...
      totalGross: invoices.reduce((sum, i) => sum + Number(i.amountGross || 0), 0),
      eligibleCount: invoices.filter(i => i.vatDeductibleStatus === VatDeductibleStatus.ELIGIBLE)
        .length,
   ...
   */
  private checkOwnership(invoice: Invoice, userId?: string, userRole?: string): void {
   ...
   * 匯出 Excel 或 CSV
   */
  async exportToExcel(query: ExportExcelDto, userId?: string, userRole?: string): Promise<Buffer> {
    // Get all invoices matching query (no pagination for export)
    const exportQuery = { ...query, page: 1, limit: 10000 };
    const { data: invoices } = await this.findAll(exportQuery, userId, userRole);
    ...
    for (const inv of invoices) {
      worksheet.addRow({
        invoiceNo: inv.invoiceNo || `${inv.invoiceTrack || ''}${inv.invoiceNumber || ''}`,
        invoiceDate: inv.invoiceDate ? new Date(inv.invoiceDate).toLocaleDateString('zh-TW') : '',
   ...
   * 依財政部媒體申報標準格式
   */
  async exportTo401(dto: Export401Dto, userId?: string, userRole?: string): Promise<string> {
    ...
    for (const inv of invoices) {
      ...
      const rocYear = (invDate.getFullYear() - 1911).toString().padStart(3, '0');
    ...
    // Add summary header
    const totalNet = invoices.reduce((sum, i) => sum + Number(i.amountNet || 0), 0);
    const totalTax = invoices.reduce((sum, i) => sum + Number(i.amountTax || 0), 0);
   ...
   * 使用 pdfmake 生成
   */
  async exportToPdf(query: ExportPdfDto, userId?: string, userRole?: string): Promise<Buffer> {
    ...
    const totalNet = invoices.reduce((sum, i) => sum + Number(i.amountNet || 0), 0);
    const totalTax = invoices.reduce((sum, i) => sum + Number(i.amountTax || 0), 0);
    const totalGross = invoices.reduce((sum, i) => sum + Number(i.amountGross || 0), 0);
        ...
        { text: ' ' },
        {
          table: {
            ...
            body: [
              ['發票號碼', '日期', '銷方', '未稅', '稅額', '含稅'],
              ...invoices.map(inv => [
                inv.invoiceNo || `${inv.invoiceTrack || ''}${inv.invoiceNumber || ''}`,
