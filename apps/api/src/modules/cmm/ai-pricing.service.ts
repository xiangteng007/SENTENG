...
export class AiPricingService {
  ...
  private readonly MARKET_FACTORS: Record<string, PriceFactor[]> = {
    steel: [
      ...
      },
      {
        ...
        impact: "negative",
        weight: 0.25,
        description: "中國減產支撐價格",
      },
      {
        factor: "基礎建設需求",
        impact: "positive",
        weight: 0.2,
        description: "公共工程需求增加",
      },
      {
        factor: "碳排成本",
        impact: "positive",
        weight: 0.15,
        description: "碳稅增加生產成本",
      },
      {
        factor: "匯率",
        impact: "neutral",
        weight: 0.1,
        description: "新台幣走勢影響進口成本",
      },
    ],
    concrete: [
      {
    ...
    wood: [
      ...
      },
      {
        ...
        impact: "neutral",
        weight: 0.25,
        description: "USD/TWD 影響",
      },
      {
        factor: "環保認證",
        impact: "positive",
        weight: 0.2,
        description: "FSC 認證成本",
      },
      {
        factor: "運輸成本",
        impact: "positive",
  ...
  predictPrice(input: PricePredictionInput): PricePrediction {
    ...
    const factors =
      this.MARKET_FACTORS[category] || this.MARKET_FACTORS.concrete;

    // 簡化的價格預測模型
    let priceChangePercent = 0;
    factors.forEach((f) => {
      const impact =
        f.impact === "positive" ? 1 : f.impact === "negative" ? -1 : 0;
