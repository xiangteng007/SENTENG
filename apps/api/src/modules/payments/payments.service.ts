...
export class PaymentsService {
  ...
  async findOne(id: string, userId?: string, userRole?: string): Promise<PaymentApplication> {
   ...
   */
  private checkOwnership(payment: PaymentApplication, userId: string, userRole: string): void {
  ...
  async create(dto: CreatePaymentDto, userId?: string): Promise<PaymentApplication> {
    // 取得合約
    ...
    const previousCumulative = lastPayment ? Number(lastPayment.cumulativePercent) : 0;
  ...
  async update(id: string, dto: UpdatePaymentDto, userId?: string): Promise<PaymentApplication> {
  ...
  async reject(id: string, reason: string, userId?: string): Promise<PaymentApplication> {
    const payment = await this.findOne(id);
  ...
  async addReceipt(dto: CreateReceiptDto, userId?: string): Promise<PaymentReceipt> {
  ...
  async exportToExcel(
    ...
    const payments = await this.findAll({ contractId, projectId, status }, userId, userRole);
